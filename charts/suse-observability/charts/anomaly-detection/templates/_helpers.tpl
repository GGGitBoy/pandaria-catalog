...
checksum/anomaly-detection-config-streams: {{ include (print $.Template.BasePath "/anomaly-detection-config-streams.yaml") . | sha256sum }}
{{- end -}}
...
{{- define "anomaly-detection.spotlight.configmap-streams" -}}
apiVersion: v1
kind: ConfigMap
metadata:
  name: {{ template "common.fullname.short" . }}-spotlight-config-streams
data:
{{- $currentScope := . }}
{{- range $configFile := tuple "streams.stsl" "views.stsl" "alerting_streams.stsl" }}
  {{- with $currentScope }}
    {{- $config := .Files.Get (list "etc/" $configFile | join "") }}
    {{- if .Values.etcoverride }}
      {{ $configFromProperty := index .Values "etcoverride" (print $configFile | replace "." "_") }}
